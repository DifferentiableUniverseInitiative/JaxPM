...
def _evolved_displacements(cosmo, mesh_shape, box_shape, a=0.5, order=2,
                           seed=7):
    k = jnp.logspace(-3, 1, 128)
    pk = jc.power.linear_matter_power(cosmo, k)
    pk_fn = lambda x: jnp.interp(x.reshape(-1), k, pk).reshape(x.shape)
    ic = linear_field(mesh_shape, box_shape, pk_fn, seed=jax.random.PRNGKey(seed))
...
def test_forces_modes_agree(simulation_config, cosmo, order):
    ...
    f_abs = pm_forces(pos, mesh_shape=mesh_shape, order=order,
                      initial_particles=None)
    f_dx = pm_forces(dx, mesh_shape=mesh_shape, order=order,
    ...
    with pytest.warns(DeprecationWarning):
        f_legacy = pm_forces(dx, mesh_shape=mesh_shape, order=order,
...
def test_forces_orders_finite_and_grad(simulation_config, cosmo, order):
    ...
    forces = pm_forces(dx, mesh_shape=mesh_shape, order=order,
                       initial_particles='uniform')
    ...
    grad = jax.grad(lambda d: jnp.sum(
        pm_forces(d, mesh_shape=mesh_shape, order=order,
                  initial_particles='uniform')**2))(dx)
...
def test_forces_deconvolution(simulation_config, cosmo):
    ...
    f_raw = pm_forces(dx, mesh_shape=mesh_shape, order='cic',
                      initial_particles='uniform', deconvolution=False)
    f_dec = pm_forces(dx, mesh_shape=mesh_shape, order='cic',
                      initial_particles='uniform', deconvolution=True)
...
def test_forces_distributed(simulation_config, cosmo, order, pdims):
    ...
    single = pm_forces(dx, mesh_shape=mesh_shape, order=order,
                       initial_particles='uniform')
    ...
    multi = pm_forces(dx_sharded, mesh_shape=mesh_shape, order=order,
                      initial_particles='uniform', halo_size=halo_size,
                      sharding=sharding)
