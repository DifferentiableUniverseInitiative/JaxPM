...
from jaxpm.painting import (cic_paint, cic_paint_dx, cic_read,  # noqa : E402
                            cic_read_dx, compensate_cic, paint, readout)
...
def test_new_cic_matches_old(simulation_config, cosmo):
    ...
    assert_allclose(paint(dx, initial_particles='uniform', order='cic'),
                    cic_paint_dx(dx), rtol=1e-4, atol=1e-4)
    assert_allclose(paint(pos.reshape(*mesh_shape, 3),
                          grid_mesh=jnp.zeros(mesh_shape), order='cic'),
                    cic_paint(jnp.zeros(mesh_shape), pos), rtol=1e-4, atol=1e-4)

    # readout -- displacement and absolute modes
    assert_allclose(readout(field, dx, initial_particles='uniform',
                            order='cic'), cic_read_dx(field, dx), rtol=1e-4,
                    atol=1e-4)
    assert_allclose(readout(field, pos.reshape(*mesh_shape, 3),
                            order='cic').reshape(-1), cic_read(field, pos),
                    rtol=1e-4, atol=1e-4)
...
def test_orders_recover_same_pk(cosmo):
    ...
    for o in ORDERS:
        ...
        ks, pk = power_spectrum(field, box_shape=box_shape, compensate_order=o,
                                shotnoise=(o, nbar))
...
def test_deconvolution_matches_compensate_cic(simulation_config, cosmo):
    mesh_shape, box_shape = simulation_config
    dx = _evolved_displacements(cosmo, mesh_shape, box_shape)
    new = paint(dx, initial_particles='uniform', order='cic', deconvolution=True)
...
def test_paint_distributed(simulation_config, cosmo, order, pdims):
    ...
    multi = paint(dx_sharded, initial_particles='uniform', order=order,
