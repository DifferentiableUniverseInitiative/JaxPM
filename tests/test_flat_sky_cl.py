...
class TestFlatSkyPowerSpectrum:
    ...
    def test_cross_power_spectrum(self):
        ...
        assert np.all(np.abs(cl_cross)
                      <= geometric_mean * 1.1)  # Allow 10% tolerance
    ...
    def test_angle_scaling(self):
        ...
        assert ell2[0] < ell1[
            0], f"ell2[0]={ell2[0]:.1f} should be < ell1[0]={ell1[0]:.1f}"
    ...
    def test_against_lenstools_auto(self):
        ...
        # Our implementation
        ell_ours, cl_ours = flat_sky_power_spectrum(kappa,
                                                    angle_deg,
                                                    ells=ells)
        ...
        if len(ell_ours) > 5 and len(ell_lenstools) > 5:
            ...
            mask_lenstools = (ell_lenstools >= min_ell) & (ell_lenstools
                                                           <= max_ell)
            ...
            if np.sum(mask_ours) > 3 and np.sum(mask_lenstools) > 3:
                ...
                cl_ours_interp = np.interp(ell_common, ell_ours[mask_ours],
                                           cl_ours_overlap)
                cl_lenstools_interp = np.interp(ell_common,
                                                ell_lenstools[mask_lenstools],
                                                cl_lenstools_overlap)
                ...
                assert abs(mean_ratio -
                           1) < 0.05, f"Mean ratio: {mean_ratio:.3f}"
    ...
    def test_against_lenstools_cross(self):
        ...
        # Our implementation
        ell_ours, cl_cross_ours = flat_sky_power_spectrum(kappa1,
                                                          angle_deg,
                                                          kappa2,
                                                          ells=ells)
        ...
        ell_lenstools, cl_cross_lenstools = conv_map1.cross(
            conv_map2, statistic="power_spectrum", l_edges=ells)
    ...
    def test_power_law_recovery(self):
        ...
        power_law = -2.5
        kappa = generate_test_map(npix=256,
                                  angle_deg=10.0,
                                  seed=42,
                                  power_law=power_law)
