...
def parse_args():
    ...
    ap.add_argument("--box", type=float, default=256.0, help="box size (Mpc/h) per side")
    ...
    ap.add_argument("--warm-cycles", type=int, default=2, help="MG V-cycles when warm-started")
    ap.add_argument("--fft-fd", action="store_true", default=True,
                    help="FFT reference uses the discrete-Laplacian operator (matches MG; fair "
                         "solver-only comparison). Default on.")
    ap.add_argument("--fft-continuum", dest="fft_fd", action="store_false",
                    help="FFT reference uses continuum -1/k^2 (JaxPM default); adds the operator gap")
    ...
    ap.add_argument("--halo-size", type=int, default=0,
                    help="CIC paint/read halo (cells) on sharded axes; >0 removes multi-device "
                         "shard-boundary seams. ~32-64 covers typical PM displacements.")
    ap.add_argument("--seed", type=int, default=0)
    ap.add_argument("--save-slab", type=int, default=0,
                    help="save only the central K z-cells of the final field (vs the full cube); "
                         "enough for slice/slab plots and far cheaper to gather/store at large N.")
    ap.add_argument("--outdir", default="fields_cmp")
    ap.add_argument("--no-plot", action="store_true")
    ap.add_argument("--warm-sweep", type=int, nargs="+", default=None,
                    help="sweep these warm-cycle counts (e.g. 1 2 3 4); reports accuracy/time tradeoff")
    ap.add_argument("--plot-only", action="store_true",
                    help="skip the sim; load saved field_{fft,mg,mgwarm}_nN.npy and make the figure (CPU)")
    ap.add_argument("--pk-sweep", type=int, nargs="+", default=None,
                    help="run FFT + cold-MG and warm-MG at each given cycle count, then compute "
...
def power_cross(d1, d2, box):
    ...
    N = d1.shape[0]
    f1 = np.fft.fftn(d1); f2 = np.fft.fftn(d2)
    ...
    binsum = lambda w: np.bincount(kbin, weights=w.ravel(), minlength=kmax + 1)[:kmax + 1]
...
def main():
    ...
    if args.plot_only:
        ...
        for s in ["fft", "mg", "mgwarm"]:
            ...
            if os.path.exists(p):
                ...
                msstep = float(open(tp).read()) if os.path.exists(tp) else float("nan")
                results[s] = (np.load(p), msstep * args.nsteps / 1e3)  # store as "total" so /nsteps recovers ms/step
        if not results:
            raise SystemExit(f"no field_*_n{N}.npy found in {args.outdir}")
        print(f"plot-only: loaded {list(results)} from {args.outdir}", flush=True)
    ...
    from jaxpm.pm import pm_forces, linear_field
    from jaxpm.distributed import fft3d, ifft3d
    from jaxpm.growth import growth_factor, growth_rate
    from jaxpm.painting import cic_paint_dx
    from jaxpm.kernels import interpolate_power_spectrum, fftk, invlaplace_kernel, gradient_kernel
    ...
    ic = jax.block_until_ready(jax.jit(lambda key: linear_field(
        mesh_shape, box, pk_fn, seed=key, sharding=sharding))(jax.random.PRNGKey(args.seed)))
    @jax.jit
    def zeldovich(ic):                          # ic passed as arg (can't close over a sharded global)
        dk = fft3d(ic)
        kv = fftk(dk)
        pot_k = dk * invlaplace_kernel(kv, fd=args.fft_fd)
        return jnp.stack([ifft3d(-gradient_kernel(kv, i) * pot_k) for i in range(3)], axis=-1)
    init_force = jax.block_until_ready(zeldovich(ic))              # (N,N,N,3) force on the grid, sharded
    dx0 = D(args.a0) * init_force                                   # Zeldovich displacement
    p0 = args.a0**2 * fr(args.a0) * E(args.a0) * dx0

    mg_kw = dict(v1=args.v, v2=args.v, omega=args.omega, agg_n=args.agg_n,
                 grad_order=args.grad_order)

    def forces_only(dx, a, solver, cycles, u0=None):
        out = pm_forces(dx, mesh_shape=mesh_shape, paint_absolute_pos=False, halo_size=args.halo_size,
                        sharding=sharding, solver=("mg" if solver != "fft" else "fft"),
                        fd=args.fft_fd,  # discrete operator on both sides => pure solver comparison
                        mg_params=dict(cycles=cycles, **mg_kw) if solver != "fft" else None,
                        u0=u0, return_potential=(solver != "fft"))
    ...
    def kdk_step(carry, i, solver, cold_cycles, warm_cycles):
        ...
        da = a_n - a
        warm = (solver == "mgwarm")
        u0 = (phi * (D(a) / D(a_steps[jnp.maximum(i - 1, 0)]))) if warm else None
    ...
    def fft_potential(dx):
        ...
        dk = fft3d(cic_paint_dx(dx, halo_size=args.halo_size, sharding=sharding))
        return ifft3d(dk * invlaplace_kernel(fftk(dk), fd=True))

    def run_one(solver, cold_cycles, warm_cycles):
        phi_seed = jnp.zeros(mesh_shape, jnp.float32, device=sharding)  # sharded (else 4GB/GPU replica)
        if solver == "mgwarm":
            phi_seed = jax.block_until_ready(fft_potential(dx0))   # one-time FFT seed at the IC
        run = jax.jit(lambda d, p, ph: jax.lax.scan(
            partial(kdk_step, solver=solver, cold_cycles=cold_cycles, warm_cycles=warm_cycles),
        ...
        field = jax.block_until_ready(cic_paint_dx(dxf, halo_size=args.halo_size, sharding=sharding))
        if args.save_slab > 0:        # keep only a thin z-slab (sliced on-device, still x/y-sharded)
            zc = field.shape[2] // 2; s = args.save_slab
    ...
    if args.pk_sweep:
        ...
        cp("[fft     ] reference"); ref = dens(run_one("fft", args.cycles, 1)[0])
        out = None
        if is_coord:
            k0, Pref, _, _, _ = power_cross(ref, ref, args.box)
            out = {"k": k0, "P_fft": Pref}
        for c in args.pk_sweep:
            for kind, solver, cc, wc in [("cold", "mg", c, 1), ("warm", "mgwarm", args.cycles, c)]:
                cp(f"[{kind} c{c}] ...")
                d = dens(run_one(solver, cc, wc)[0])
                if is_coord:
                    k0, Pxx, _, _, r = power_cross(d, ref, args.box)
                    out[f"P_{kind}{c}"] = Pxx; out[f"r_{kind}{c}"] = r
                    j = len(k0) // 2
    ...
    if args.warm_sweep:
        ...
        for wc in args.warm_sweep:
            ...
            cp(f"[warm wc{wc}] {dt/args.nsteps*1e3:6.2f} ms/step   relL2={rel:.3e}   "
               f"speedup {tf/dt:.2f}x" + ("  *** beats FFT ***" if dt < tf else ""))
        if is_coord and not args.no_plot:
            make_sweep_plot(sweep, tf / args.nsteps * 1e3, (relc, tc / args.nsteps * 1e3), args)
        return
...
def make_pk_plot(out, args):
    ...
    k = out["k"]; Pf = out["P_fft"]; m = k > 0
    cycles = sorted({int(key.split("cold")[1]) for key in out if key.startswith("P_cold")})
    ...
    for c, col in zip(cycles, cmap):
        ax[0].loglog(k[m], out[f"P_warm{c}"][m], "-", color=col, label=f"warm wc{c}")
        ax[0].loglog(k[m], out[f"P_cold{c}"][m], "--", color=col, label=f"cold c{c}")
    ax[0].set_xlabel("k [h/Mpc]"); ax[0].set_ylabel("P(k)"); ax[0].set_title("power spectrum")
    ax[0].legend(fontsize=7, ncol=2); ax[0].grid(alpha=.3, which="both")
    # (b) transfer ratio P/P_fft
    for c, col in zip(cycles, cmap):
        ax[1].semilogx(k[m], out[f"P_warm{c}"][m]/np.maximum(Pf[m],1e-300), "-", color=col, label=f"warm wc{c}")
        ax[1].semilogx(k[m], out[f"P_cold{c}"][m]/np.maximum(Pf[m],1e-300), "--", color=col)
    ax[1].axhline(1, c="k", lw=.8); ax[1].set_ylim(0.8, 1.2)
    ax[1].set_xlabel("k [h/Mpc]"); ax[1].set_ylabel(r"$P/P_{\rm FFT}$"); ax[1].set_title("transfer ratio")
    ax[1].legend(fontsize=7); ax[1].grid(alpha=.3, which="both")
    # (c) cross-correlation 1-r(k)
    for c, col in zip(cycles, cmap):
        ax[2].loglog(k[m], 1-out[f"r_warm{c}"][m], "-", color=col, label=f"warm wc{c}")
        ax[2].loglog(k[m], 1-out[f"r_cold{c}"][m], "--", color=col)
    ax[2].set_xlabel("k [h/Mpc]"); ax[2].set_ylabel(r"$1-r(k)$"); ax[2].set_title("decorrelation vs FFT")
    ax[2].legend(fontsize=7); ax[2].grid(alpha=.3, which="both")
    fig.suptitle(f"{args.mesh}$^3$, box={args.box:g} Mpc/h, {args.nsteps} steps  "
                 f"(solid=warm, dashed=cold)")
    o = f"{args.outdir}/pk_n{args.mesh}.png"; fig.savefig(o, dpi=150); print(f"  wrote {o}", flush=True)
...
def make_sweep_plot(sweep, t_fft, cold, args):
    ...
    wc = [s[0] for s in sweep]; ts = [s[1] for s in sweep]; rel = [s[2] for s in sweep]
    fig, ax = plt.subplots(1, 2, figsize=(11, 4.2), constrained_layout=True)
    ax[0].plot(wc, rel, "o-", label="warm MG")
    ax[0].axhline(cold[0], ls="--", c="gray", label=f"cold MG (c{args.cycles})")
    ax[0].set_xlabel("warm cycles"); ax[0].set_ylabel("final-field rel-L2 vs FFT")
    ax[0].set_yscale("log"); ax[0].set_xticks(wc); ax[0].legend(); ax[0].grid(alpha=.3)
    ax[1].plot(ts, rel, "o-")
    for w, t, r, _ in sweep:
        ax[1].annotate(f"wc{w}", (t, r), fontsize=8, xytext=(3, 3), textcoords="offset points")
    ax[1].axvline(t_fft, ls="--", c="k", label=f"FFT {t_fft:.1f} ms/step")
    ax[1].scatter([cold[1]], [cold[0]], c="gray", marker="s", label=f"cold MG c{args.cycles}")
    ax[1].set_xlabel("ms / step"); ax[1].set_ylabel("final-field rel-L2 vs FFT")
    ax[1].set_yscale("log"); ax[1].legend(); ax[1].grid(alpha=.3)
    fig.suptitle(f"warm-cycles accuracy/speed tradeoff  ({args.mesh}^3, {args.nsteps} steps, "
                 f"{jax.device_count()} GPU)")
    out = f"{args.outdir}/warm_sweep_n{args.mesh}.png"
    fig.savefig(out, dpi=150); print(f"  wrote {out}", flush=True)
...
def make_figure(results, args):
    ...
    nz = results[order[0]][0].shape[2]      # actual z-extent (full cube, or a pre-saved slab)
    slab = min(nz, max(1, N // 16))         # project a z-slab (thin slices are mostly-empty voids ->
    zc = nz // 2                            # blocky log-clamped patches); slab = std cosmic-web viz
    z0, z1 = zc - slab // 2, zc + slab // 2
    proj = lambda f: np.log10(np.maximum(f[:, :, z0:z1].sum(axis=2) / slab, 1e-3))
    ncol = len(order)
    fig, axes = plt.subplots(2, ncol, figsize=(4.2 * ncol, 8), constrained_layout=True)
    ...
    for j, s in enumerate(order):
        ...
        im = axes[0, j].imshow(proj(f).T, origin="lower", cmap="magma", vmin=vmin, vmax=vmax)
        ...
        im2 = axes[1, j].imshow(d.T, origin="lower", cmap="RdBu_r", vmin=-dm, vmax=dm)
    ...
    fig.suptitle(f"PM density (log), {slab}-cell z-slab, {N}^3, {args.nsteps} steps", fontsize=14)
